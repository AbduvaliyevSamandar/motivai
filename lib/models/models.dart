...
class Task {
  ...
  Color  get color  => C.catColors[category] ?? C.primary;

  String get diffLabel => const {
    'easy': 'Oson', 'medium': "O'rta", 'hard': 'Qiyin', 'expert': 'Expert',
  ...
  factory Task.fromJson(Map<String, dynamic> j) => Task(
    ...
    completedAt:      j['completed_at'] != null
        ? DateTime.tryParse(j['completed_at']) : null,
  );
...
class LbEntry {
  ...
  const LbEntry({
    ...
    required this.level, required this.streak,
  });
...
class Achievement {
  ...
  const Achievement({
    ...
    required this.emoji, required this.rarity, required this.bonusPoints,
...
class ChatMsg {
  ...
  factory ChatMsg.fromJson(Map<String, dynamic> j) => ChatMsg(
    ...
    role:      j['role'] ?? 'user',
    content:   j['content'] ?? '',
    timestamp: j['ts'] != null ? DateTime.parse(j['ts']) : DateTime.now(),
    tasks:     (j['tasks'] as List?)?.map((t) => TaskSuggestion.fromJson(t)).toList(),
    isError:   j['err'] ?? false,
  );
...
class TaskSuggestion {
  final String title, description, category, difficulty;
  final int    durationMinutes, estimatedPoints;
  bool isSelected;
